...
<div class="menu-size" style="height:365px;">
    ...
    <div class="content mt-0">
        <?php
            ...
            $query = "SELECT f.group_id AS group_no, c.name AS attendee_name FROM attendance_tbl a INNER JOIN locales_tbl b ON a.locale_id = b.id INNER JOIN members_tbl c ON a.member_id = c.id INNER JOIN batches_tbl d ON a.batch_id = d.id INNER JOIN platforms_tbl e ON a.platform_id = e.id INNER JOIN group_members_tbl f ON a.member_id = f.member_id WHERE e.short_name = 'Zoom' AND CURRENT_TIME() BETWEEN d.start_time AND d.end_time AND d.day = (WEEKDAY(CURRENT_DATE())) AND DATE(a.date_created) = CURRENT_DATE() ORDER BY f.group_id ASC";
